...
def process_partition(args, definitions, code_to_group, id_to_group, group_to_id,
    ...
    with open(output_listfile_path, "w") as listfile:
        listfile_header = "patient_id,stay_id,stay,period_length," + ",".join(codes_in_benchmark)
        listfile.write(listfile_header + "\n")
        
        for (patient_id, stay_id, stay_filename, period_length, y_true) in xty_triples:
            labels = ','.join(map(str, y_true))
            listfile.write('{},{},{},{:.6f},{}\n'.format(patient_id, stay_id, stay_filename, period_length, labels))
