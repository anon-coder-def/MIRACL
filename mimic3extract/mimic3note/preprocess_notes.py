...
def extract_to_csv(args, partition, eps=1e-6):
    ...
    notes_df = pd.DataFrame(columns=['PatientID', 'ICUSTAY_ID', 'Recordtime', 'AdmissionTime', 'DischargeTime', 'Text'])
    for filename in tqdm(filenames, desc='Iterating over notes in all data'):
        ...
        text_df = pd.DataFrame({'PatientID': patient_id, 'ICUSTAY_ID': int(icustay), 'Recordtime': time, 'AdmissionTime': intime, 
                                'DischargeTime': outtime, 'Text': text})
        notes_df = pd.concat([notes_df, text_df], ignore_index=True)
        
    
    notes_df.to_csv(os.path.join(output_dir, f"mimic3_{partition}_notes.csv"), index=False)
